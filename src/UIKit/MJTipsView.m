...
@property (nonatomic, assign) CGFloat spaceOfWindow;
@property (nonatomic, assign) CGFloat spaceOfTipsView;
...
- (id)init {
    if (self = [super init]) {
        self.spaceOfWindow = kSpacOfWindow;
        self.spaceOfTipsView = kSpacOfTipsView;
        self.font = kFont;
...
- (void)updateUI {
    // 汉字左右对齐
    if (kIsOpenTextLeftRightAlignment) {
    ...
    //
    CGFloat space = (self.spaceOfWindow + self.spaceOfTipsView) * 2;
    CGFloat x = self.spaceOfWindow;
    // 计算单行长、宽
    CGSize txtSize = [self.currentContent sizeWithAttributes:@{NSFontAttributeName:self.font}];
    ...
    if (txtSize.width > supFrame.size.width - space) {
        ...
        CGRect rect = [self textRect:self.font displaySize:size text:self.currentContent];
        txtH = rect.size.height;
    } else {
        x = (supFrame.size.width - (txtSize.width + self.spaceOfTipsView * 2)) / 2.0;
    }
    //
    CGFloat y = (supFrame.size.height - (txtH + self.spaceOfTipsView * 2)) / 2.0; // 默认居中
    //
    switch (self.currentAlignment) {
        case 0:
            break;
        case 1:
            y = navigationBarH + self.spaceOfWindow;
            break;
        case 2:
            y = supFrame.size.height - tabBarH - self.spaceOfWindow - (txtH + self.spaceOfTipsView * 2);
            break;
    ...
    //
    self.frame = CGRectMake(x, y, supFrame.size.width - x * 2, txtH + self.spaceOfTipsView * 2);
    //
    self.label.text = self.currentContent;
    self.label.frame = CGRectMake(self.spaceOfTipsView, self.spaceOfTipsView, self.frame.size.width - self.spaceOfTipsView * 2, self.frame.size.height - self.spaceOfTipsView * 2);
}
