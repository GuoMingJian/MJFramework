...
#define MJMBMainThreadAssert() NSAssert([NSThread isMainThread], @"MJMBProgressHUD needs to be accessed on the main thread.");

CGFloat const MJMBProgressMaxOffset = 1000000.f;

static const CGFloat MJMBDefaultPadding = 4.f;
static const CGFloat MJMBDefaultLabelFontSize = 16.f;
static const CGFloat MJMBDefaultDetailsLabelFontSize = 12.f;


@interface MJMBProgressHUD () {
    // Deprecated
...
@interface MJMBProgressHUDRoundedButton : UIButton
@end


@implementation MJMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MJMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MJMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MJMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MJMBProgressHUD *hud = (MJMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = MJMBProgressHUDAnimationFade;
    _mode = MJMBProgressHUDModeIndeterminate;
    _margin = 16.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MJMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MJMBProgressHUDAnimationZoom) {
        type = animatingIn ? MJMBProgressHUDAnimationZoomIn : MJMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MJMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MJMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MJMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MJMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
        if (self.bgAlpha == 0) {
            self.bgAlpha = 1.f;
        }
        CGFloat alpha = animatingIn ? self.bgAlpha : 0.f;
...
- (void)done {
    ...
    MJMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MJMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MJMBBackgroundView *backgroundView = [[MJMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MJMBProgressHUDBackgroundStyleSolidColor;
    ...
    MJMBBackgroundView *bezelView = [MJMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MJMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:MJMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;
    
    UIButton *button = [MJMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MJMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MJMBRoundProgressView class]];
    
    MJMBProgressHUDMode mode = self.mode;
    if (mode == MJMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == MJMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MJMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MJMBProgressHUDModeDeterminate || mode == MJMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MJMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MJMBProgressHUDModeAnnularDeterminate) {
            [(MJMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == MJMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MJMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MJMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MJMBRoundProgressView class]]) {
        MJMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[MBProgressHUD class], nil];
#else
        appearance = [MJMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MJMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MJMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MJMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MJMBBarProgressView class]]) {
        MJMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MJMBBarProgressView appearanceWhenContainedIn:[MJMBProgressHUD class], nil];
#else
        appearance = [MJMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MJMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MJMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MJMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    MJMBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MJMBDefaultPadding : 0.f;
...
- (void)setMode:(MJMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MJMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MJMBRoundProgressView
...
@implementation MJMBBarProgressView
...
@implementation MJMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = MJMBProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = MJMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(MJMBProgressHUDBackgroundStyle)style {
    if (style == MJMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = MJMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    MJMBProgressHUDBackgroundStyle style = self.style;
    if (style == MJMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MJMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation MJMBProgressHUDRoundedButton
